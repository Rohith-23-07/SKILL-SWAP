...
const connectDB = async () => {
  ...
  try {
    ...
    try {
      ...
      if (count === 0) {
        console.log(' [Skill Swap DB] Seeding initial sample skills to Cloud Database...');
        await Skill.insertMany(sampleSkills.map(s => {
          const { id, ...rest } = s;
          return rest;
        }));
        console.log(' [Skill Swap DB] Initial sample skills seeded successfully!');
      }
    } catch (seedErr) {
      console.warn(' [Skill Swap DB] Warning during initial seed:', seedErr.message);
    }
  } catch (error) {
    console.warn('\n--------------------------------------------------------------');
    console.warn(` [Skill Swap DB] Cloud MongoDB connection failed: ${error.message}`);
    console.warn(' -> Automatically falling back to Local In-Memory Mode.');
    console.warn(' -> The application will continue running smoothly with sample data.');
    console.warn('--------------------------------------------------------------\n');
    isConnected = false;
  }
};
